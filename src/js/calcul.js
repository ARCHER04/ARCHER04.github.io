...
function qunAns(){
    let=sayt=prompt("Тип сайта: 1-Сайт-визитка , 2-Корпоративный сайт , 3-Интернет-магазин");
    if(sayt !=1 && sayt!=2 && sayt!=3){
        alert("Нет варианта ответа");
        qunAns();
        return;
    }
    let=dis=prompt("Дизайн: 1-Шаблонный , 2-Уникальный , 3-WOW-Дизайн");
    if(dis !=1 && dis!=2 && dis !=3){
        alert("Нет варианта ответа");
        qunAns();
        return;
    }
    let=adapt=prompt("Адаптивность: 1-Только ПК , 2-Только мобильный сайт , 3-ПК + Мобильный");
